...
const toCamel = function(s) {
  return s.charAt(0).toUpperCase() + s.slice(1).replace(/([-_\.][a-z])/ig, function($1) {
...
function convertArray(arr) {
  ...
  arr.toString = function() {
    if(arrayMode === 'length') return this.length.toString()
  };
  return arr;
}
...
function convertOperation(op, verb, path, pathItem, obj, api) {
  ...
  if(!operation.notes) {
    ...
    operation.notes.toString = function() {
      return ''
    };
  }
  ...
  for(let r in op.responses) {
    ...
    if(response.content) {
      ...
      operation.returnType = !entry.isPrimitiveType ? "models." + toCamel(entry.dataType) : entry.dataType;
      ...
      if(schema !== null) {
        ...
        walkSchema(schema, {}, wsGetState, function(schema, parent, state) {
          ...
          if(entry.name && state.depth <= 1) {
            ...
            entry.datatypeWithEnum = s + '.' + toCamel(entry.name) + 'Enum';
            entry.enumName = toCamel(entry.name) + 'Enum';
            model.hasEnums = true;
